<?php include("BD.php") ?>
<?php include("Header.php"); ?>

<body class="container">
    <main>
        <div class="card">
            <div class="card-body">
                <!-- Solicitando el ingreso de datos -->
                    <div class="mb-3">
                        <form method="post">
                            <input type="text" name="id" hidden value="<?php ;?>">
                            <!-- Solicitando Categoria -->
                                <select class="form-select form-select-ls" name="categoria" id="categoria">
                                    <!--<option selected></option>-->
                                    <option value="Papeleria" selected>Papeleria</option>
                                    <option value="Variedades">Variedades</option>
                                    <option value="Electronico">Electronico</option>
                                    <option value="Argenis">Argenis</option>
                                    <option value="Estefania">Estefania</option>
                                </select>
                            <!-- Solicitando Nombre -->
                                <input type="text" class="form-control" name="objeto" id="objeto" aria-describedby="helpId" placeholder="Nombre del objeto" value="">
                            <!-- Solicitando Descripcion -->
                                <input type="text" class="form-control" name="descripcion" id="descripcion" aria-describedby="helpId" placeholder="Descripcion del objeto">
                            <!-- Solicitando Valor al por mayor -->
                                <div class="input-group">
                                    <span class="input-group-text">$</span>
                                    <input type="number" class="form-control" name="valorObjetoEmpresa" id="valorObjetoEmpresa" aria-describedby="helpId" placeholder="Valor de la compra ($)">
                                </div>
                            <!-- Solicitando Cantidad disponible -->
                                <input type="number" class="form-control" name="cantidadObjeto" id="cantidadObjeto" aria-describedby="helpId" placeholder="Cantidad disponible">
                            <!-- Solicitando Valor -->
                                <div class="input-group">
                                    <span class="input-group-text">$</span>
                                    <input type="number" class="form-control" name="valorObjeto" id="valorObjeto" aria-describedby="helpId" placeholder="Valor de la venta ($)">
                                </div>
                            <br>
                            <input class="btn btn-primary" type="submit" value="Agregar">
                        </form>
                    </div>
                <!-- Solitando el Ingeso de datos-->

                <!-- Mostrando los datos -->
                    <div class="table-responsive row tbl-fixed">
                        <table class=" table table-striped table-hover table-borderless table-primary align-middle" style="text-align:center">
                            <thead class="table-dark align-middle">
                                <tr>
                                    <th>Categoria</th>
                                    <th>Objeto</th>
                                    <th>Descripcion</th>
                                    <th>Valor coste</th>
                                    <th>Cantidad disponible</th>
                                    <th>Valor venta</th>
                                    <th>Accion</th>
                                </tr>
                            </thead>
                            <tbody>
                                <?php foreach ($resultado as $registro) { ?>
                                    <tr>
                                        <td hidden><?php echo $registro["mId"]; ?></td>
                                        <td><?php echo $registro["mCategoria"]; ?></td>
                                        <td><?php echo $registro["mNombre"]; ?></td>
                                        <td><?php echo $registro["mDescripcion"]; ?></td>
                                        <td>$ <?php echo number_format($registro["mPorMayor"]); ?></td>
                                        <td><?php echo number_format($registro["mDisponible"]); ?></td>
                                        <td>$ <?php echo number_format($registro["mValor"]); ?></td>
                                        <td>
                                            <h6 class="float-start">
                                                <a href="Editar.php?eId=<?php echo $registro["mId"];?>"><span
                                                class="badge bg-warning text-dark"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-pen" viewBox="0 0 16 16"><path d="m13.498.795.149-.149a1.207 1.207 0 1 1 1.707 1.708l-.149.148a1.5 1.5 0 0 1-.059 2.059L4.854 14.854a.5.5 0 0 1-.233.131l-4 1a.5.5 0 0 1-.606-.606l1-4a.5.5 0 0 1 .131-.232l9.642-9.642a.5.5 0 0 0-.642.056L6.854 4.854a.5.5 0 1 1-.708-.708L9.44.854A1.5 1.5 0 0 1 11.5.796a1.5 1.5 0 0 1 1.998-.001zm-.644.766a.5.5 0 0 0-.707 0L1.95 11.756l-.764 3.057 3.057-.764L14.44 3.854a.5.5 0 0 0 0-.708l-1.585-1.585z"/></svg></span></a>
                                            </h6>
                                            <h6 class="float-end">
                                                <a href="?dId=<?php echo $registro["mId"];?>"><span
                                                class="badge bg-danger"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-trash3" viewBox="0 0 16 16"><path d="M6.5 1h3a.5.5 0 0 1 .5.5v1H6v-1a.5.5 0 0 1 .5-.5ZM11 2.5v-1A1.5 1.5 0 0 0 9.5 0h-3A1.5 1.5 0 0 0 5 1.5v1H2.506a.58.58 0 0 0-.01 0H1.5a.5.5 0 0 0 0 1h.538l.853 10.66A2 2 0 0 0 4.885 16h6.23a2 2 0 0 0 1.994-1.84l.853-10.66h.538a.5.5 0 0 0 0-1h-.995a.59.59 0 0 0-.01 0H11Zm1.958 1-.846 10.58a1 1 0 0 1-.997.92h-6.23a1 1 0 0 1-.997-.92L3.042 3.5h9.916Zm-7.487 1a.5.5 0 0 1 .528.47l.5 8.5a.5.5 0 0 1-.998.06L5 5.03a.5.5 0 0 1 .47-.53Zm5.058 0a.5.5 0 0 1 .47.53l-.5 8.5a.5.5 0 1 1-.998-.06l.5-8.5a.5.5 0 0 1 .528-.47ZM8 4.5a.5.5 0 0 1 .5.5v8.5a.5.5 0 0 1-1 0V5a.5.5 0 0 1 .5-.5Z"/></svg></span></a>
                                            </h6>
                                        </td>
                                    </tr>
                                <?php } ?>
                            </tbody>
                            <tfoot>

                            </tfoot>
                        </table>
                    </div>
            </div>
        </div>
    </main>
</body>

<?php include("Footer.php"); ?>